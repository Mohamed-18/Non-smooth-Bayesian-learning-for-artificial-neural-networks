    ...
    for epoch in range(epochs):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            ...
            for ni in range(1, Nmcmc + 1):
                ...
                LikelihoodRatio = Compute_E_theta(y_hat_old, y, zOld, Lambda/Sigma2) - \
                                  Compute_E_theta(logits, y, zStar, Lambda/Sigma2)

                # test model
                mse = tf.keras.losses.MeanSquaredError()
                accuracy_train = accracy(y_hat_star, y_batch_train)
                loss_train = mse(y_batch_train, y_hat_star).numpy()
